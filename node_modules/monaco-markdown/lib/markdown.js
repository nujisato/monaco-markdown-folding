...
var language = {
  ...
  latexKeywords: _latex.all,
  latexBeginKeywords: _latex.begin_args,
  tokenizer: {
    root: [// headers (with #)
    ...
    [/\{+[^}]+\}+/, 'string.target'], [/(!?\[)((?:[^\]\\]|@escapes)*)(\]\([^\)]+\))/, ['string.link', '', 'string.link']], [/(!?\[)((?:[^\]\\]|@escapes)*)(\])/, 'string.link'], //inline math
    [/(\$\$)([^$]*)(\$\$)/, [{
      token: 'comment.math',
      bracket: '@open'
    }, {
    ...
    latex: [[/(\\begin)({)/, ['keyword', {
      ...
      cases: {
        '@latexKeywords': 'keyword',
        '@default': ''
      }
    }]],
    latexBeginArg: [[/\w+/, {
      cases: {
        '@latexBeginKeywords': 'keyword',
        '@default': ''
      }
    }], [/}/, {
      token: '',
